...
- **personality-web** — Next.js 16 personality-test app
  ([`marshallku/personality-test`](https://github.com/marshallku/personality-test)),
  served at `personality.irang.me`. Calls a `kagi-serve` sidecar (same pod) for
  LLM narrative interpretation and caches results in an on-pod SQLite file. It
  lives in the irang namespace only to reuse `irang-secret` (Kagi creds) and
  `ghcr-secret` — it shares no DB or API with the rest of irang.
...
| NodePorts | 30500 frontend, 30501 api | 30504 api, 30505 admin, 30508 web, 30509 personality |
...
├── personality/{deployment,service}.yaml    # personality.irang.me + kagi sidecar
│   └── argocd-application.yaml.example       # Application CR (apply manually, once)
...
| `personality` | `irang.me` | HTTP | `personality-web.irang.svc.cluster.local:3000` |
...
Added after the original irang bootstrap. It rides on the same namespace,
`irang-secret` (Kagi creds) and `ghcr-secret`, so there is no new secret or
Infisical project — only four manual steps:

1. **CI token.** In `marshallku/personality-test` repo settings → Secrets,
   add `MANIFEST_REPO_TOKEN` (a PAT with `contents:write` on
   `marshallku/manifest`). Without it the build succeeds but the image-tag
   commit-back fails and the deployment stays on `:prd-placeholder`.
2. **Image pull.** The image is `ghcr.io/marshallku/personality-test`
   (private). `ghcr-secret` already pulls `ghcr.io/marshallku/kagi` in this
   namespace, so it works as-is — or make the package public to drop the
   dependency.
3. **ArgoCD.** Register the app once (CR is not synced from the repo):

   ```sh
   kubectl apply -f kubernetes/service/irang/personality/argocd-application.yaml.example
   ```
4. **Cloudflare.** Add the `personality.irang.me` Public Hostname (row in the
   table above) to the `sssup` tunnel.

Verify:

```sh
kubectl -n irang rollout status deploy/personality-web --timeout=3m
# Both containers up — web + kagi-serve sidecar:
kubectl -n irang get pod -l app=personality-web
curl -sS -o /dev/null -w "%{http_code}\n" https://personality.irang.me   # expect 200
```

> **Heads-up:** `KAGI_PROFILE_ID` (`09fd3173-…`, the narrative Custom Assistant)
> must exist under the Kagi account whose `KAGI_EMAIL`/`KAGI_PASSWORD` live in
> `irang-secret`. If the profile was created under a different personal account
> during local dev, either recreate it under the irang account or point the env
> at the right UUID — otherwise `/api/interpret` returns 500.
